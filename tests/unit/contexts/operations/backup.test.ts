...
import { saveInstanceConfig } from "@/platform/config/instance";
...
describe("backup/backup", () => {
  ...
  test("createBackup packages config, secrets, and database", async () => {
    ...
    const { SecretStore } = await import("@/contexts/access/infrastructure/secrets/store");
  ...
  test("restoreBackup restores into destination home", async () => {
    const { paths, dbPath } = setupHome();
    const store = new (await import("@/contexts/access/infrastructure/secrets/store")).SecretStore(db!, paths);
    ...
    const restoredStore = new (await import("@/contexts/access/infrastructure/secrets/store")).SecretStore(
      restoredDb,
