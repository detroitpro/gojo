...
For `pull-request` tasks, gojo builds the GitHub PR title/body from `.gojo/handoff.json`. Prefer a short `summary` (title material) plus optional `assets` with `role: "pr-body"` pointing at a verbose markdown file (e.g. `.gojo/assets/pr-body.md`), and/or `role: "pr-title"`. Without assets, gojo synthesizes the body from `summary` / `decisions` / lists.
