...
Legacy CLI handlers in `src/transports/cli/index.ts` still return unpaged sets for `gojo run list`, `gojo agent list --project`, and `gojo schedule list` (see removal-backlog R5). Registry use cases (`execution.run.list`, `catalog.agents.list`, `catalog.schedules.list`) and the matching HTTP routes are paged. `gojo project work`, `gojo integration list`, and `gojo approval list` return the first page only (default limit 25).
...
- Workspace branch/worktree names are `gojo/run/<agent>/<project>/<date>/run-<fullRunId>` (optional `-aN` attempt suffix) under `$GOJO_HOME/worktrees`. The reserved `gojo/run/…` namespace keeps human topic branches (`gojo/<topic>`) from colliding with nested run refs; branch-protection allowlists should match `gojo/run/<agent>/*`. Full ULID + project slug avoids collisions when many schedules fire in the same millisecond. Orphan paths under the worktrees root are reclaimed before `git worktree add`, and terminal runs (including failed/timed-out/canceled) remove their worktree; `gojo instance sweep-worktrees` (also on daemon start) clears leftovers. Profile `timeout` (e.g. `45m`) is honored for adapter wall-clock limits.
- Merge/babysit agents declare `mergePolicy` in `gojo.yaml` (`includeAgents: "*"` or an explicit name list, optional `excludeAgents`). At run time gojo injects a **Gojo merge scope** section listing allowed head prefixes `gojo/run/<agent>/…` derived from sibling agents — do not duplicate branch laundry lists in prompt files. `includeAgents: "*"` skips the merge agent itself and defaults to excluding `self-heal`.
