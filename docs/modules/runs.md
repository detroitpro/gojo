...
- Injects `GOJO_API_URL`, `GOJO_API_TOKEN`, `GOJO_RUN_ID`, `GOJO_TASK_ID`, and `GOJO_PROJECT_ID` into agent env. `agent-run-*` tokens are short-lived, restricted to `POST /runs/:id/progress` for that run, and revoked when the attempt finishes. Healers diagnose failed runs via `gojo run list|inspect|artifacts` (or `$GOJO_HOME/artifacts/<runId>/`) — not via the agent token.
