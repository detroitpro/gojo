...
4. **HTTP handlers stay thin.** Orchestration lives in `contexts/*/application/` use cases and `platform/` composition—not ad-hoc logic in `transports/http/` routers.
...
```text
cli ──────────────► platform (registry dispatch)
api ──────────────► platform (registry dispatch)
...
| UI or router embedding cron math | `contexts/scheduling/` + repository ports |
| Domain modules importing transports for manifest sync | `contexts/catalog/application/project-sync.ts` (shared by CLI, API, coordinator) |
